function  plot_layer_SUA(SUA, zscore, smooth_data, plot_errorbars)
...
%     plot_errorbars = true;
    ...
    % Make figure
    figure('Position', [1000 432 423 906])
    ...
    for iLayer = 1:n_layers
        for iCell = 1:n_cell_types
            ...
            for ii = 1:length(conditions)
                data = SUA.(cell_type).(layer).(conditions{ii});
        
                if zscore == 1 % z-score based on average firing for all conditions
                    all_data = [];
                    for iii = 1:length(conditions)
                        all_data = [all_data; SUA.(cell_type).(layer).(conditions{iii})];
                    end
            ...
            % Plot data
            subplot(n_layers, n_cell_types, iPlot)
            hold on
            for ii = 1:length(conditions)
                if plot_errorbars == 1
            ...
            title(sprintf('%s %s cells (n = %d)', layer, spike_type, size(data,2)))
            xlabel('Time (msec)')
            ylabel('Firing Rate')
            if iPlot == 8 && show_legend
