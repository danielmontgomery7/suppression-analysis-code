...
function plot_areaerrorbar(data, options)
    ...
    % Default options
    default_ops.handle     = figure(1);
    default_ops.color_area = [128 193 219]./255;    % Blue theme
    default_ops.color_line = [ 52 148 186]./255;
%     default_ops.color_area = [243 169 114]./255;    % Orange theme
%     default_ops.color_line = [236 112  22]./255;
    default_ops.alpha      = 0.5;
    default_ops.line_width = 2;
    default_ops.error      = 'sem';
    
    if(nargin<2)
        options = default_ops;
    else
        options = catstruct(default_ops, options);
    end
