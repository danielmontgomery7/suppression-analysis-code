function intan_information = read_Intan_RHD2000_file(filename)

% function slightly modifed from the available read_Intan_RHD2000_file;
% namely, the elimination of sending stuff to the base workspace and
% instead returning it as a struct to the calling workspace
...
% Dustin edit: use function input instead of GUI to get filename
last_delim = find(filename == '/', 1, 'last');
path = filename(1:last_delim);
file = filename(last_delim+1:end);
...
% Dustin addition: move to calling function's workspace then end function
filename = file;
intan_information = struct( ...
    'filename', [path, file], ...
    'path', path, ...
    'notes', notes, ...
    'frequency_parameters', frequency_parameters,...
    'eval_board_mode', eval_board_mode, ... % Dustin edit: added to ensure data stored correctly
    'data_file_main_version_number', data_file_main_version_number, ... % Dustin edit: added to ensure data stored correctly
    'data_file_secondary_version_number', data_file_secondary_version_number); % Dustin edit: added to ensure data stored correctly);
if (data_file_main_version_number > 1)
    intan_information.reference_channel = reference_channel;
end
if (num_amplifier_channels > 0)
    intan_information.amplifier_channels = amplifier_channels;
    intan_information.spike_triggers = spike_triggers;
end
if (num_aux_input_channels > 0)
    intan_information.aux_input_channels = aux_input_channels;
end
if (num_supply_voltage_channels > 0)
    intan_information.supply_voltage_channels = supply_voltage_channels;
end
if (num_board_adc_channels > 0)
    intan_information.board_adc_channels = board_adc_channels;
end
if (num_board_dig_in_channels > 0)
    intan_information.board_dig_in_channels = board_dig_in_channels;
end
if (num_board_dig_out_channels > 0)
    intan_information.board_dig_out_channels = board_dig_out_channels;
end
return
